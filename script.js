...
const CONFIG = {
  ...
  // TODO: Replace '#' with your actual YouFibre referral sign-up URL.
  // Example: 'https://www.youfibre.com/refer?code=KCR5KH'
  // All referral buttons read from this single value.
  referralUrl: '#',
 ...
 *
 * Called via onclick attributes on referral <a> elements in the HTML.
 * Note: the click still navigates to the referral URL normally after tracking.
 */
...
function initStickyCta() {
  ...
  const observer = new IntersectionObserver(
    ...
    {
      // Fire when the hero fully leaves the viewport
      threshold: 0,
    }
  );
 ...
 * Only updates if CONFIG.referralUrl is a real URL (not the '#' placeholder).
 */
 ...
 *
 * The overlay HTML lives in index.html; this function only attaches
 * event listeners. Call once after DOMContentLoaded.
 */
function initLightbox() {
  ...
  // Remember which element triggered the lightbox so focus can be restored on close
  ...
  function closeLightbox() {
    ...
    // Clear the src after the CSS transition finishes (0.25s) to free memory
    setTimeout(() => {
      imgEl.src = '';
      imgEl.alt = '';
    }, 260);
  ...
  document.querySelectorAll('.lightbox-trigger').forEach((trigger) => {
    trigger.addEventListener('click', (e) => {
      ...
      if (!img) return;
      // Use the anchor href as the full-res source; fall back to the img src
...
/**
 * localStorage key for persisting the user's consent choice.
 * @type {string}
 */
...
function loadGA4() {
  ...
  // Inject the GA4 async loader
  const script = document.createElement('script');
  script.async = true;
  script.src = `https://www.googletagmanager.com/gtag/js?id=${measurementId}`;
  document.head.appendChild(script);

  // Initialise the data layer and fire the initial page_view
 ...
 *
 * - Prior consent 'accepted'  → load GA4 silently, skip banner.
 * - Prior consent 'declined'  → do nothing, skip banner.
 * - No recorded choice        → show the banner after a short delay.
 */
